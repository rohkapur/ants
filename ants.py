...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        i = 0
        while begin != beehive:
            if bee_selector(begin.bees) != None and self.min_range <= i and i <= self.max_range:
                return bee_selector(begin.bees)
            else:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        death = self.damage + amount
        for i in self.place.bees[:]:
            if amount >= self.health:
                i.reduce_health(death)
            else:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chewing == 0:
            beee = bee_selector(self.place.bees)
            if beee is not None:
                self.chewing = self.chew_duration
                beee.reduce_health(beee.health)
        else:
            self.chewing -= 1 
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if not self.first:
            ...
            super().action(gamestate)
            while behind:
                if behind.ant and not behind.ant.repeat:
                    behind.ant.damage *= 2
                    behind.ant.repeat = True
                if behind.ant and behind.ant.is_container() and behind.ant.contained_ant is not None and not behind.ant.contained_ant.repeat:
                    behind.ant.contained_ant.damage *= 2
                    behind.ant.contained_ant.repeat = True
                behind = behind.exit
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem EC
        super().reduce_health(amount)
        if self.health <= 0 and self.first:
            bees_win()
        # END Problem EC
